  ...
  const renderTableContent = () => {
    if (loading) {
      return Array.from({ length: 5 }).map((_, index) => (
        <TableRow
          key={`skeleton-${index}`}
          sx={{ animation: 'fadeIn 0.3s ease both', animationDelay: `${index * 0.05}s` }}
        >
          <TableCell>
          ...
          <TableCell>
            <Skeleton variant="rectangular" width={56} height={26} sx={{ borderRadius: '6px' }} />
          </TableCell>
      ...
      return (
        <TableRow>
          <TableCell colSpan={6} align="center">
            ...
            >
              <AssignmentLateIcon
                sx={{ fontSize: 56, mb: 2, opacity: 0.25, color: COLORS.textMuted }}
              />
              <Typography
                variant="h5"
                sx={{
      ...
      >
        ...
        <DialogContent>
          <Typography>
            ...
            <Typography component="span" sx={{ fontWeight: 700, color: COLORS.textPrimary }}>
              {selectedRecord?.name}
            </Typography>
            ?
          </Typography>
